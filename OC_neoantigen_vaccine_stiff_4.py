# -*- coding: utf-8 -*-
"""
Created on Mon Oct 31 15:10:27 2022
...
import latextable
#%%
def Patient_info_1(patient,h_p_low,h_p_up,h_a_low,h_a_up):
    ...
    #h_a_up = 1*dose_a
    'Weight Parameters'
    A = 10 #?same magnintue as T(0) or higher
    A_4 = 1e-1
    A_8 = 1e-1
    B = 1e-20
    C = 1e-20
...
for i in range(NN)
    dose_p = dose_p_all[i]
    j0patients_oc[i+1] = Patient_info_1(i+1,10,dose_p*3,dose_a/2,dose_a*3)

'Max Control'
j0patients_max={}
for i in range(NN):
    dose_p = dose_p_all[i]
    j0patients_max[i+1] = Patient_info_1(i+1,dose_p,dose_p,dose_a,dose_a)

'3-Quarter max Control'
j0patients_3quarter={}
for i in range(NN):
    dose_p = dose_p_all[i]
    j0patients_3quarter[i+1] = Patient_info_1(i+1,3*dose_p/4,3*dose_p/4,3*dose_a/4,3*dose_a/4)

'Half max Control' 
j0patients_half={}
for i in range(NN):
    dose_p = dose_p_all[i]
    j0patients_half[i+1] = Patient_info_1(i+1,dose_p/2,dose_p/2,dose_a/2,dose_a/2)

'Quarter max Control'
j0patients_quarter={}
for i in range(NN):
    dose_p = dose_p_all[i]
    j0patients_quarter[i+1] = Patient_info_1(i+1,dose_p/4,dose_p/4,dose_a/4,dose_a/4)

'No control'
j0patients_none={}
for i in range(NN):
    dose_p = dose_p_all[i]
    j0patients_none[i+1] = Patient_info_1(i+1,0,0,0,0)
    #%%
'Total number of activated T-cells (ACD4+ + ACD8) by patient'
tau = [0,3,7,14,21,83,139]
plt.rcParams.update({'font.size': 30,})
fig, axs = plt.subplots(3, 2,figsize=(28,18))
fig.suptitle('Total number of activated T-cells (ACD4$^+$ + ACD8$^+$) by patient using  $J_2$')
for i in range(len(axs.flat)):
    l = len(j0patients_oc[i+1][1]) 
    t = patients_oc[i+1][0]
    [T, N_CD4, N_CD8, A_CD4, A_CD8] = j0patients_oc[i+1][1][l-5:l]
    ...
    if i<4:
        ...
        axs.flat[i].annotate('Booster', xy=(tau[6], 0 ), xycoords='data',
                             ...
                             horizontalalignment='right', verticalalignment='top')
fig.tight_layout()
plt.savefig(sim_path+'j0Total_T_cells.eps', format='eps',dpi=1200)
    #%%
...
l = len(j0patients_oc[patient][1]) 
t = j0patients_oc[patient][0]
[T, N_CD4, N_CD8, A_CD4, A_CD8] = j0patients_oc[patient][1][l-5:l]
Total_cells = A_CD4 + A_CD8
...
j0peptide_dose = np.zeros((6,len(t)))
j0peptide_dosec = np.zeros((6,len(t)))
for i in range(6):
    [t,state,adjoint,control_hp, control_ha,rtime] = j0patients_oc[i+1]
    [t,statec,adjointc,control_hpc, control_hac,rtimec] = j0patients_max[i+1]
    for j in range(len(t)):
        j0peptide_dose[i][j] = molartomass(control_hp[j],peptide_mw[i])
        j0peptide_dosec[i][j] = molartomass(control_hpc[j],peptide_mw[i])
        
j0adjuvant_dose = np.zeros([6,len(t)])
j0adjuvant_dosec = np.zeros([6,len(t)])
for i in range(6):
    j0adjuvant_dose[i] = j0patients_oc[i+1][4]
    j0adjuvant_dosec[i] = j0patients_max[i+1][4]
...
for j in range(6):
    for i in range(len(tau)):
        j0peptide_doses[j][i] = integrate.trapezoid(rho*j0peptide_dose[j][2*i*100:((2*i+1)*100)-1],
                                                  t[2*i*100:((2*i+1)*100)-1])
        j0adjuvant_doses[j][i] = integrate.trapezoid((rho*j0adjuvant_dose[j][2*i*100:((2*i+1)*100)-1])/1000,
                                                  t[2*i*100:((2*i+1)*100)-1])
        j0peptide_dosesc[j][i] = integrate.trapezoid(rho*j0peptide_dosec[j][2*i*100:((2*i+1)*100)-1],
                                                  t[2*i*100:((2*i+1)*100)-1])
        j0adjuvant_dosesc[j][i] = integrate.trapezoid((rho*j0adjuvant_dosec[j][2*i*100:((2*i+1)*100)-1])/1000,
                                                  t[2*i*100:((2*i+1)*100)-1])
#%%
barWidth = 0.25
...
fig.tight_layout()
plt.savefig(sim_path+'j0peptide_adjuvant.eps', format='eps',dpi=1200)
#%%
...
for i in range(len(axs.flat)):
    ...
    if i<4:
        ...
        axs.flat[i].annotate('Booster', xy=(tau[6], 0 ), xycoords='data',
                             ...
                             horizontalalignment='right', verticalalignment='top')
fig.tight_layout()
plt.savefig(sim_path+'j0Tumor_diam.eps', format='eps',dpi=1200)
#%%
patient = 2
...
axs.flat[3].bar(br1, j0peptide_doses[i] , color ='royalblue', width = barWidth,
        edgecolor ='grey', label ='Peptide')
axs.flat[3].hlines(y = dose_a/1000, xmin= 0, xmax = 6, 
                   colors= 'k',linestyle = '--')#,label='max polyICLC = 0.5')
axs.flat[3].bar(br2, j0adjuvant_doses[i] , color ='g', width = barWidth,
        edgecolor ='grey', label ='Adjuvant')
...
fig.tight_layout()
plt.savefig(sim_path+'j0_t_cells.png', format='png',dpi=300)
#%% 
def patient_t_T_cells(patient,function):
    ...
    [t,z] = function[patient][0:2]
    
    D = d*((z[l+5]/z[l+1])**lambdaa)/(s+(z[l+5]/z[l+1])**lambdaa)
    
    pM_I = N_A * sum((z[7+i*4] for i in range(n)))*1e-12
    pM_II = N_A * sum((z[4*n + 7 + i*4] for i in range(m)))*1e-12
    ...
    'New T-cells'
    new_ACD4 = integrate.trapezoid(sigma_4*F_p4*z[l+2]*Psi_4 + rho_4*Phi_4 *z[l+4]
                                   + c_4*z[l+1]/(a_1+z[l+1])*z[l+4],t)
    new_ACD8 = integrate.trapezoid(c*(D**2*z[l+1]**2)/(a+D**2*z[l+1]**2)*z[l+5] + c_8*z[l+4]*z[l+1] 
                  + sigma_8*F_p8*z[l+3]*Psi_8 + rho_8*Phi_8 *z[l+5],t)
    newcells = interp1d(t,(sigma_4*F_p4*z[l+2]*Psi_4 + rho_4*Phi_4 *z[l+4]
                       + c_4*z[l+1]/(a_1+z[l+1])*z[l+4])
                    +(c*(D**2*z[l+1]**2)/(a+D**2*z[l+1]**2)*z[l+5] 
                      + c_8*z[l+4]*z[l+1] + sigma_8*F_p8*z[l+3]*Psi_8 
                      + rho_8*Phi_8 *z[l+5]))
    Total_new_tcells = new_ACD4 + new_ACD8
    "New Tumor Cells"
     
    Total_new_tumor = integrate.trapezoid(r*z[l+1]*(1-z[l+1]/K_T),t)
    
    return [Total_new_tcells, Total_new_tumor, newcells]
#%%
...
Tdiam = np.zeros(len(T))
Tcdiam = np.zeros(len(Tc))
Tq3diam = np.zeros(len(Tq3))
Thdiam = np.zeros(len(Th))
Tqdiam = np.zeros(len(Tq))
Tndiam = np.zeros(len(Tn))

for j in range(len(t)):
    Tdiam[j] = diam(T[j])
    Tcdiam[j] = diam(Tc[j])
    Tq3diam[j] = diam(Tq3[j])
    Thdiam[j] = diam(Th[j])
    Tqdiam[j] = diam(Tq[j])
    Tndiam[j] = diam(Tn[j])
...
Total_vaccine = sum(j0peptide_doses[patient-1])+sum(j0adjuvant_doses[patient-1])        
Total_vaccinec = sum(j0peptide_dosesc[patient-1])+sum(j0adjuvant_dosesc[patient-1])        
...
ax.set_xticklabels(labels=['Optimal','Max','3-Quarter-Max','Half-Max',
                           'Quarter-Max','None'],rotation = 25)
...
[Total_cells, T, newcells] = patient_t_T_cells(patient,j0patients_oc)
[Total_cellsc, Tc,newcells] = patient_t_T_cells(patient,j0patients_max)
[Total_cellsq3, Tq3,newcells] = patient_t_T_cells(patient,j0patients_3quarter)
[Total_cellsh, Th,newcells] = patient_t_T_cells(patient,j0patients_half)
[Total_cellsq, Tq,newcells] = patient_t_T_cells(patient,j0patients_quarter)
[Total_cellsn, Tn,newcells] = patient_t_T_cells(patient,j0patients_none)

Total_tumors = np.array([T,Tc,Tq3,Th,Tq,Tn])
#Total_tumors = Total_tumors/max(Total_tumors)

Total_Tcells = np.array([Total_cells,Total_cellsc,Total_cellsq3,
                         Total_cellsh,Total_cellsq,Total_cellsn])
#Total_Tcells = Total_Tcells/max(Total_Tcells)
...
ax.plot(Vaccine_dose, Total_tumors,'o',color = 'blue')
ax.plot(vspan, Tumor_response(vspan),color = 'blue', label = 'Tumor Response')
ax.set_ylabel('Total New Tumor Cells',color = 'blue')
ax.set_xticks(ticks=Vaccine_dose)
ax.set_xticklabels(labels=['Optimal','Max','3-Quarter-Max','Half-Max',
                           'Quarter-Max','None'],rotation = 25)
ax.set_xlabel('Total Drug Concentration')
...
ax2.plot(Vaccine_dose, Total_Tcells,'o' ,color = 'orange')
ax2.plot(vspan, Tcell_response(vspan), color = 'orange', label = 'T-cell Response')
ax2.set_ylabel('Total New T-cell Response',color = 'orange')
ax2.set_xlabel('Drug Concentration')
b = str('{:.2e}'.format(round(Total_Tcells[0],0)))
fT = Total_Tcells[0]
ax2.annotate(b, xy=(Vaccine_dose[0], fT ), xycoords='data',
...
fig.tight_layout()
#%%
'New T cells, Tumor (cells and diameter), vaccine dosage'
patient = 1
peptides = [13,17,14,14,20,20]
barWidth = 0.25
max_dose = [3.9,5.1,4.2,4.2,6,6]
...
t = j0patients_oc[patient][0]
[T, N_CD4, N_CD8, A_CD4, A_CD8] = j0patients_oc[patient][1][l-5:l]
Total_cells = patient_t_T_cells(patient,j0patients_oc)[2]
[Tc, N_CD4c, N_CD8c, A_CD4c, A_CD8c] = j0patients_max[patient][1][l-5:l]
Total_cellsc = patient_t_T_cells(patient,j0patients_max)[2] 
[Th, N_CD4h, N_CD8h, A_CD4h, A_CD8h] = j0patients_half[patient][1][l-5:l]
Total_cellsh = patient_t_T_cells(patient,j0patients_half)[2]
[Tn, N_CD4n, N_CD8n, A_CD4n, A_CD8n] = j0patients_none[patient][1][l-5:l]

axs.flat[0].set_title('New T cells')
axs.flat[0].plot(t, Total_cells(t), color='blue', label = 'Optimal Control')
axs.flat[0].plot(t, Total_cellsc(t), color='orange', label = 'Max Control')
axs.flat[0].plot(t, Total_cellsh(t), linestyle=':',color='orange', label = 'Half max Control')
axs.flat[0].set_xlabel('Days')
axs.flat[0].legend(loc='best', fancybox=True, shadow=True,fontsize = 12)
axs.flat[0].vlines(x = tau, ymin= 0, ymax = max(Total_cellsc(t)), colors= 'g',linestyle = '--')
...
axs.flat[1].plot(t, Tn, linestyle='--',color = 'red', label = 'No control, T(200) = '+str('{:.2e}'.format(Tn[len(t)-1])))
axs.flat[1].set_xlabel('Days')
axs.flat[1].fill_between(t,Tn,T,step='pre',alpha=0.4)
axs.flat[1].legend(loc='upper left', fancybox=True, shadow=True,fontsize = 12)
axs.flat[1].vlines(x = tau, ymin= 0, ymax = max(Tn), colors= 'g',linestyle = '--')
b = str('{:.2e}'.format(T[len(t)-1]))
fT = T[len(t)-1]
...
axs.flat[3].set_title('Patient '+str(i+1)+' ('+str(peptides[i])+' immunizing peptides)')
axs.flat[3].bar(br1, j0peptide_doses[i] , width = barWidth,
                 label ='Peptide',bottom = j0adjuvant_doses[i])
axs.flat[3].hlines(y = dose_a/1000+1, xmin= 0, xmax = 6, 
                   colors= 'k',linestyle = '--')#,label='max polyICLC = 0.5')
axs.flat[3].bar(br1, j0adjuvant_doses[i] , color ='g', width = barWidth,
                edgecolor ='grey', label ='Adjuvant')
axs.flat[3].set_ylim(0,3*max_dose[i]+3*4*0.5)
axs.flat[3].set_xlabel('Treatment (5-Prime and 2-Booster)', fontweight ='bold')
axs.flat[3].set_ylabel('mg/4ml')
#axs.flat[i].set_ylabel('(Peptide + PolyICLC) per 4ml', fontweight ='bold')
axs.flat[3].set_xticks([r + barWidth for r in range(len(tau))],
...
fig.tight_layout()
plt.savefig(sim_path+'j0new_t_cells.eps', format='eps',dpi=1200)
#%%
'Vaccine dose on Immune Response and Cancer Thickness for all patients'

tau = [0,3,7,14,21,83,139]
plt.rcParams.update({'font.size': 17,"axes.formatter.useoffset":False})
fig, axs = plt.subplots(3, 2,figsize=(16,14))
fig.suptitle('Immune and Tumor Responses to Vaccine Doses')
for i in range(len(axs.flat)):
    patient = i+1
    ax = axs.flat[i]
    l = len(j0patients_oc[patient][1]) 
    ...
    #ax2.legend(loc='best', fancybox=True, shadow=True,fontsize = 14)
    fig.tight_layout()
    plt.savefig(sim_path+'j0immune_tumor_response_to_dose_response.eps', format='eps',dpi=1200)
    #%%
...
table2 = [['Patient ID', '$T^{\text{opt}}(200)$', 'T^{\text{ct}}(200)',
           ...
           'J_2^{\text{opt}}/J_2^{\text{ct}}?',
           'J_2^{\text{opt}}/J_2^{\text{half-ct}}?']] 
for i in range(6):
    [t,state,adjoint,control_hp, control_ha,rtime] = j0patients_oc[i+1]
    [T, N_CD4, N_CD8, A_CD4, A_CD8] = state[len(state)-5:len(state)]
    T_f_opt.append(T[len(t)-1])
    J_2_control[i] = integrate.trapezoid(A*T + A_4*A_CD4 + A_8*A_CD8 +
                                         B*control_hp**2 + C*control_ha**2,t) + T[len(t)-1]
    
    [t,state,adjoint,control_hp, control_ha,rtime] = j0patients_max[i+1]
    [T, N_CD4, N_CD8, A_CD4, A_CD8] = state[len(state)-5:len(state)]
    T_f_max.append(T[len(t)-1])
    J_2_max[i] = integrate.trapezoid(A*T + A_4*A_CD4 + A_8*A_CD8 +
                                     B*control_hp**2 + C*control_ha**2,t) + T[len(t)-1]
    
    [t,state,adjoint,control_hp, control_ha,rtime] = j0patients_half[i+1]
    [T, N_CD4, N_CD8, A_CD4, A_CD8] = state[len(state)-5:len(state)]
    T_f_half.append(T[len(t)-1])
    J_2_half[i] = integrate.trapezoid(A*T + A_4*A_CD4 + A_8*A_CD8 +
                                     B*control_hp**2 + C*control_ha**2,t) + T[len(t)-1]
    
    [t,state,adjoint,control_hp, control_ha,rtime] = j0patients_none[i+1]
    [T, N_CD4, N_CD8, A_CD4, A_CD8] = state[len(state)-5:len(state)]
    T_f_no.append(T[len(t)-1])
    J_2_no[i] = integrate.trapezoid(A*T + A_4*A_CD4 + A_8*A_CD8 +
                                     B*control_hp**2 + C*control_ha**2,t) + T[len(t)-1]
...
for i in range(len(peptide_doses[patient-1])):
    pep.append(round(j0peptide_doses[patient-1][i],1))
    adj.append(round(j0adjuvant_doses[patient-1][i],1))
...
print(latextable.draw_latex(table))
    #%%
    'Activated T cells, Tumor (cells and diameter), vaccine dose'
    tau = [0,3,7,14,21,83,139]
    plt.rcParams.update({'font.size': 17,})
    patient = 1
    i = patient-1
    ...
    axs.flat[3].bar(br1, j0peptide_doses[i] , color ='royalblue', width = barWidth,
            edgecolor ='grey', label ='Peptide')
    axs.flat[3].hlines(y = (3/2)*dose_a/1000, xmin= 0, xmax = 6, 
                       colors= 'k',linestyle = '--')#,label='max polyICLC = 0.5')
    axs.flat[3].bar(br2, j0adjuvant_doses[i] , color ='g', width = barWidth,
            edgecolor ='grey', label ='Adjuvant')
    axs.flat[3].set_ylim(0,3*max_dose[i])
    ...
    axs.flat[3].legend(loc='best', fancybox=True, shadow=True)
    fig.tight_layout()
    plt.savefig(sim_path+'j2_t_cells.eps', format='eps',dpi=600)